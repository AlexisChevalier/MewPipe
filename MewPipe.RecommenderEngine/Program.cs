...
namespace MewPipe.RecommenderEngine
{
    public class Program
    {
        ...
        private static void ProcessOneVideoRecommendations(Guid id)
        {
            ...
            Console.Out.WriteLine("[INFO][" + DateTime.Now.ToLongDateString() + "] Recommendations for video " + id + " in database updated in " + watch.ElapsedMilliseconds + " ms");
        }
        ...
        private static void ProcessAllRecommendations()
        {
            while (!_closing)
            {
                ...
                foreach (var video in realData.VideoRatingDatas)
                {
                    ...
                    var socialResults = pearsonEngine.GetTopMatches(realData.VideoUserRatingDatas, realData.VideoRatingDatas.First(r => r.Value.VideoId == video.Key).Value);
                    Console.Out.WriteLine(smallWatch.ElapsedMilliseconds + " ms elapsed for social results");
                    smallWatch.Restart();
                    var contentResults = contentEngine.GetTopMatches(realData.VideoRatingDatas, realData.VideoRatingDatas.First(r => r.Value.VideoId == video.Key).Value);
                ...
                watch.Stop();
                Console.Out.WriteLine("[INFO][" + DateTime.Now.ToLongDateString() + "] All Recommendations in database updated in " + watch.ElapsedMilliseconds + " ms");
            }
        }
