...
using MewPipe.Accounts.OpenID.Provider;
    ...
    internal abstract class OpenIDAuthenticationHandlerBase<T> : AuthenticationHandler<T> where T : OpenIDAuthenticationOptions
    {
        ...
        protected override async Task ApplyResponseChallengeAsync()
        {
            ...
            if (challenge != null)
            {
                ...
                var body = await Request.GetBodyParameters();
                var providerUrl = body["OpenIdLoginUrl"];
                if (!String.IsNullOrEmpty(providerUrl) && providerUrl != Options.ProviderDiscoveryUri)
                {
                    Options.ProviderDiscoveryUri = providerUrl;
                    Options.ProviderLoginUri = null;
                }
                ...
                HttpContext.Current.SetErrorMessage("Can't find a valid OpenID provider with this URL");
